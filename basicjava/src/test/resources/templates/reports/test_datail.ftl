...
<#if testSuiteMap?exists>
    ...
    <div class="row-fluid">
        ...
        ${key}测试详细：
           ...
           <#if flag>
               <span class="label label-success" ondbclick="hideOrShowScreenshot('#${key?replace(" ","_")}')">收起或者展开详细内容</span>
           <#else>
               <span class="label label-danger" ondbclick="hideOrShowScreenshot('#${key?replace(" ","_")}')">收起或者展开详细内容</span>
           </#if>

        </h4>

        <div id="${key?replace(" ","_")}" class="table-responsive">
